...
REPORT_PERIOD = 4 # send a frame every 60 sample (1 hour)
...
def send_coap_message(sock, destination, uri_path, message):
    ...
    coap = CoAP.Message()
    coap.new_header(type=CoAP.CON, code=CoAP.POST)
    ...
    if destination[0] == "LORAWAN_STACK":
        answer= send_ack(coap)
    else:
...
if (SERVER=="LORAWAN_STACK") and True:
    coap=l_coap_s.test_send_req()
    l_coap_s.sleep(60) # send msg and handl msg ack and dw msg
    resp=l_coap_s.request(coap).response
    if (resp): 
...
while False:
    ...
    print("Iteration {} -- time stamp = {} --".format(i,time.time()))
    print (t, p, h, m)
